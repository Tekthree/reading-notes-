...
- You will need a form ``` <form> ``` element whenever you want to collect information from visitors

- The form take information in as name/value pairs

- Each form control is given a name, and the text the user types in or the values of the options they select are sent to the server
. Su
- In the new HTML5, they have added in form elements that make it easier for visitors to fill in forms. Such as Email, url input, and search input.
...
- List markers can be given different appearances using the list-stlye-type and list-style image properties

- Table cells can have different borders and spaing in different bowsers, that are ways that you can control that to make more consistant across browsers.

- Forms are easier to use if the form controls are vertically alignes using CSS.

- Forms benefit from styles that mnake them feel more interactive.

- The cursor style allows yout to control the type of mouse cursor that should be displayed to users

- Fieldset helps determin the edges of a form

- ![table](https://mdn.mozillademos.org/files/14583/numbers-table.png)

## Events

- Events are the browser's way of indicating when something has happened. For example and mouse click or page load

- Binding is the process of stating which event you are waiting to happen, and which element you are waiting for that event to happen to.

- When an event occurs on an element, it can trigger a javascript function. This creates the interactivity on the internet or in software programs

- You can use event delegation to monitor for events that happen of all of the children of an element.

- The most commonly used events are W3C DOM events

// from  W3Schoolswww.w3schools.com
```
